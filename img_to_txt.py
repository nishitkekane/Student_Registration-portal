...
def textExtracting(thress_img, flag, img):
    ...
    if flag == 0:
        data = pytesseract.image_to_string(thress_img, lang='eng+hin')
        return data
    if flag == 1:
        data = pytesseract.image_to_string(img, lang='eng+hin')
        return data
...
def main():
    file = "/home/nishitk/Documents/adhar_final/WhatsApp Image 2024-03-23 at 05.31.36 (1).jpeg"
    npimg = np.fromfile(file, np.uint8)
    file = cv2.imdecode(npimg, cv2.IMREAD_COLOR)
    li = ocrHandler(file)
    return li

unstructured_dat = main()
unstructured_dat_json = json.dumps(unstructured_dat)
print(unstructured_dat)
